...
const CTE_PREFIX = `
	...
	"user" AS (SELECT 0 WHERE 0), session AS (SELECT 0 WHERE 0),
	account AS (SELECT 0 WHERE 0), verification AS (SELECT 0 WHERE 0),
	channel_links AS (SELECT 0 WHERE 0), link_requests AS (SELECT 0 WHERE 0),
	conversation_context AS (SELECT 0 WHERE 0)`;

const SCHEMA = `
job_postings(id, url, title, content, created_at)  -- global catalog of saved JDs; content is the full JD text
job_applications(id, job_id -> job_postings.id, status, fit, created_at)
  -- status: considering|applied|interviewing|offer|rejected|withdrawn; fit: apply|stretch|skip, NULL = not evaluated
profile_notes(id, summary, content, created_at, removed_at)  -- removed_at NULL = active; content NULL = summary says it all
company_boards(id, ats, slug, created_at)  -- global ATS boards, e.g. ats='greenhouse'
